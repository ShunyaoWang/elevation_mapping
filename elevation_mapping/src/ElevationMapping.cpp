...
ElevationMapping::ElevationMapping(ros::NodeHandle& nodeHandle)
    : nodeHandle_(nodeHandle),//elevation map mapping
      map_(nodeHandle),       //map store?elavation map object
      robotMotionMapUpdater_(nodeHandle),//map uodate from robot motion
      ...
      ignoreRobotMotionUpdates_(false)
{
  ...
  readParameters();//
  //WSHY: callback function to update elevation map from PointCloud2 data and Robot motion
  pointCloudSubscriber_ = nodeHandle_.subscribe(pointCloudTopic_, 1, &ElevationMapping::pointCloudCallback, this);
  //WSHY: check if robot pose topic exists
  if (!robotPoseTopic_.empty()) {
    //WSHY: use messagefilter class to subcribe and get robot pose cache according cache size
  ...
  //WSHY: set a Timer to update map from motion and fuse the map periodically(fuse has some puzzle,need to think again)
  ...
  //WSHY: advertise fusion services
  ...
  //WSHY: set a Timer to publish fuse the map periodically
    ...
    const sensor_msgs::PointCloud2& rawPointCloud)
{
  ...
  pcl_conversions::toPCL(rawPointCloud, pcl_pc);//WSHY: convert sensor_msgs/PointCloud2 to PCL/Pointcloud2

  PointCloud<PointXYZRGB>::Ptr pointCloud(new PointCloud<PointXYZRGB>);
  pcl::fromPCLPointCloud2(pcl_pc, *pointCloud);//WSHY: Convert a PCLPointCloud2 binary data blob into a pcl::PointCloud<T> object
  ...
  // Process point cloud.
  // WSHY: compute variance of 1-d kalmalfilter using sensor data,where the sensor model decide the
  //       sensor cov matrix, the rotation var in robotPoseVarinces decide the rotation noise of sensor
...
bool ElevationMapping::updatePrediction(const ros::Time& time)
{
  ...
  HomTransformQuatD robotPose;//WSHY: How update the rotation
  //WSHY: the pose include position and orientation, guess there are two reload function
  //being called,to update both position and orientation, this calling construct the robotPose
  //class,the position_ and rotation template in this class are loaded as
  //Position<double, 3>, RotationQuaternion<double>, but How the constucting work?
...
bool ElevationMapping::updateMapLocation()
{
  ...
  //WSHY:tranform the base location(form a track) to map frame and move the origin of elevation map to
  // a new position(Where the trackPoint_ be updated?)
  try {
